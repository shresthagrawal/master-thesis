...
Blockchains suffer from a scalability problem. During periods of high demand, transaction fees have repeatedly spiked and networks have become slow or unusable. The most widely adopted response has been Layer-2 (L2) scaling solutions, such as rollups. Rollups move execution off-chain while relying on a single sequencer to order transactions, execute them, and extract the new state. The sequencer posts these transaction batches along with the resulting states to the underlying Layer-1 blockchain. While effective in practice, rollups come with trade-offs. Validity-proof designs require a centralized sequencer to produce proofs of correctness for each state transition~\cite{zk-rollups}, while optimistic designs weaken finality guarantees, as the sequencer can post incorrect states and users must wait out a dispute period~\cite{arbitrum}.

An alternative approach is to relax the requirement for strong-consensus or total ordering altogether. This tradeoff is well understood in distributed systems, where asynchronous execution sacrifices strong consistency (all nodes agree on the state at every step) for eventual consistency (nodes may temporarily diverge but eventually converge to the same state) to achieve better performance. Conflict-Free Replicated Data Types (CRDTs)~\cite{crdt} formalize this property: they are data structures whose operations commute by design, so concurrent updates from different replicas always converge to the same state without coordination. CRDTs are widely used in collaborative editing (e.g., Google Docs, Figma) and distributed databases (e.g., Riak, Redis) where low-latency updates across replicas matter more than strict ordering.

In the blockchain setting, it has been shown that not all applications require consensus: in particular, BFT payment systems can be implemented without total ordering. This insight has led to a line of work on consensusless Byzantine fault-tolerant protocols. FastPay~\cite{fastpay} provides a particularly simple solution for payments: clients collect validator votes to finalize transfers within a single network round trip, avoiding consensus entirely.

These protocols achieve confirmation within a single network round trip and support throughput several orders of magnitude higher than consensus-based systems -- for example, FastPay~\cite{fastpay} reports over 80,000 transactions per second compared to Ethereum's approximately 15 TPS~\cite{ethereum-tps} or Solana's 4,000 TPS under real-world conditions~\cite{solana-tps}. However, despite these advantages, consensusless protocols have not seen meaningful production adoption.
...
\begin{description}
    ...
    Consensusless protocols like FastPay~\cite{fastpay} support only payments. Extending them to richer applications requires identifying which data structures -- such as CRDTs -- can be safely used without global ordering.
...
We start by introducing the system model, cryptographic primitives, and security properties we require from our construction (Chapter~\ref{chap:background}). We then survey the state of the art in BFT consensus and consensusless protocols to understand their trade-offs and limitations (Chapter~\ref{chap:related-work}). We also examine view-change mechanisms in consensus protocols, as they closely relate to recovery from equivocation. Since our protocol builds on FastPay, we begin by presenting a simplified version of FastPay and then progressively extend it with recovery support (Chapter~\ref{chap:protocol}). We provide complete pseudocode for both validators and clients. We then establish safety and liveness through a formal security analysis (Chapter~\ref{chap:security-analysis}). Finally, we describe our prototype implementation and evaluate its performance, confirming that recovery adds minimal overhead to the fast path (Chapter~\ref{chap:implementation}). We conclude with a summary and directions for future work (Chapter~\ref{chap:conclusion}).
