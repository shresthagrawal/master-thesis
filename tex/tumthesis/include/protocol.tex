...
\begin{itemize}
    ...
    \item \textbf{Notarization threshold:} $n - 3f$. A transaction with an $(n-3f)$-quorum is \emph{notarized}. This weaker threshold is used for recovery.
\end{itemize}
...
Algorithm~\ref{alg:validator} shows the simplified validator logic, based on the FastPay pseudocode~\cite{fastpay} without sharding.
...
\label{sec:recovery-overview}

We progressively extend the FastPay construction to allow for full recovery. On a high level,
...
\draw[thick, dashed] (3.6,-3) rectangle (5.5,-2);
% Position 1: blue
\node[bt] at (3.3,-2.5) {\checkmark};
% Position 2: blue
...
\draw[thick, dashed] (2,-3) rectangle (5.3,-2);
% Position 1: blue (solid only)
\node[bt] at (1.7,-2.5) {\checkmark};
...
To maintain backward compatibility with existing Ethereum wallets and tooling, we leverage a precompile contract: we designate a special address $\mathsf{recovery\_contract}$ for handling recovery
transactions. The transaction structure remains unchanged -- clients simply send a transaction to the recovery contract address.
...
\textbf{Validator Protocol.}
Algorithms~\ref{alg:validator-recovery-tx},~\ref{alg:validator-recovery-vote}, and~\ref{alg:validator-recovery-cert} present the extended validator
...
When a validator receives a vote (Algorithm~\ref{alg:validator-recovery-vote}, \textsc{OnVote}), it verifies
...
has reached the notarization quorum of $n-3f$ (line~\ref{line:check-no-majority}), the validator signs $\bot$
(line~\ref{line:sign-bot}), signaling that no transaction can be finalized at this nonce. If notarization
...
\begin{algorithm}[ht]
\caption{Validator with Recovery - Vote Handling}\label{alg:validator-recovery-vote}
\begin{algorithmic}[1]
...
\begin{algorithm}[ht]
\caption{Validator with Recovery - Certificate Processing}\label{alg:validator-recovery-cert}
\begin{algorithmic}[1]
\Procedure{ProcessCertificate}{$a, k$} \label{line:process-cert}
    ...
    \If{$k = \mathsf{account}[a].\mathsf{nonce}$}
        \If{$q < n - 3f$ \textbf{and} $|\mathsf{votes}[a][k]| \geq n - f$ \textbf{and} not voted $\bot$} \label{line:check-no-majority}
            \Comment{no notarization quorum but have $n-f$ votes}
...
\Procedure{ExecuteTransfer}{$tx$}
    \State $\mathsf{account}[tx.\mathsf{sender}].\mathsf{balance} \mathrel{-}= tx.\mathsf{amount}$ \label{line:balance-sub}
    \State $\mathsf{account}[tx.\mathsf{recipient}].\mathsf{balance} \mathrel{+}= tx.\mathsf{amount}$ \label{line:balance-add}
\EndProcedure
\Statex
...
\section{Extending to Other Consensusless Protocols}
\label{sec:applicability}

The recovery mechanism presented in this thesis extends naturally to more expressive consensusless protocols.
