...
(line~\ref{line:check-finality}), the validator can safely finalize the transaction and update the finalised counter (line~\ref{line:update-finalised}).
...
\Procedure{OnCertificate}{$C$}
    ...
    \State $(q, tx) \gets \text{max quorum in } C$ \label{line:extract-quorum}
    \State $a \gets C.\mathsf{account}$
    \Statex
    \If{$k = \mathsf{account}[a].\mathsf{nonce}$}
        \If{$q < n - 3f$} \label{line:check-no-majority}
            ...
            \State broadcast $\langle \mathsf{vote}, a, k, \bot \rangle_v$ to $\mathcal{V}$ \label{line:sign-bot}
    ...
    \If{$q \geq n - f$ \textbf{and} $k > \mathsf{account}[a].\mathsf{finalised}$ \textbf{and} $tx \neq \bot$} \label{line:check-finality}
        ...
        \Comment{original tx must be same as finalized or next (+1)}
    ...
    \EndIf
    \Statex
    \Comment{process current nonce if certificate exists}
    \If{$\exists$ certificate $C'$ for nonce $\mathsf{account}[a].\mathsf{nonce}$}
        \State \Call{OnCertificate}{$C'$}
    \EndIf
\EndProcedure
\Statex
