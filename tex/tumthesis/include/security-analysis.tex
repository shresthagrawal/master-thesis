...
\textbf{Fast-path progress.} We first show that non-equivocating clients make progress within one round trip. Intuitively, if a client sends a single transaction for a nonce, all honest validators will vote for it, and the client will be able to collect enough votes to finalize.
...
\textbf{Main liveness result.} Combining the above, we conclude that healing clients always make progress and that the protocol remains responsive - confirmation depends on actual network delay, not on a known upper bound.
