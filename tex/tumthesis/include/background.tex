...
\]
Conflict-Free Replicated Data Types (CRDTs)~\cite{crdts} formalize this property:
they are data structures whose operations commute by design, so concurrent updates
from different replicas always converge to the same state without coordination.
CRDTs are widely used in collaborative editing (e.g., Google Docs, Figma) and
distributed databases (e.g., Riak, Redis) where low-latency updates across replicas
