...
\za{Why are the consensus properties not here? The title says consensus, but it's not defined here. Maybe you need safety and liveness of SMR, as you have defined the ledgers based on the SMR definitions, and below in the next section you literally mention safety and liveness.}

\section{Asynchronous Protocols}
\label{sec:async-protocols}

In this thesis, we use \emph{asynchronous protocols} to mean protocols that avoid global consensus and total ordering, not necessarily protocols that assume a fully asynchronous network. The key idea is that many workloads do not require a globally agreed total order to be safely executed. \za{NO!!!!! You cannot do this, its a very bad idea. Name them consensusless!!!!!!!!}
...
\za{this is a building block, unlike the other sections, which are definitional. We need to either make another section "building blocks" or find some way to make it work in this section.}
...
We distinguish two client behaviors: \emph{honest clients} follow the protocol, while \emph{Byzantine clients} may deviate arbitrarily. Under relaxed termination, Byzantine clients that introduce conflicts are not guaranteed liveness - the protocol may halt progress for that client's state. This is a fundamental limitation: safety is preserved unconditionally, but liveness is sacrificed in the presence of conflicts. \za{this should only be part of the adversarial model in the preliminaries, it has no place here. merge or delete if already mentioned}
